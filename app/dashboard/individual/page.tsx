 ...
 * 
 * Vollständiges Dashboard für Individual-Plan-User.
 * Dies ist das bisherige Dashboard mit allen Features:
 * - Chat
 * - Dokumente
 * - Mail
 * - Startseite
 ...
 * - Karten
 * 
 * WICHTIG: Diese Route ist nur für User mit Plan "individual" zugänglich.
 * Siehe Guard-Logik unten.
 */
...
import DashboardInbox from '@/src/components/dashboard/DashboardInbox';
import DashboardTeams from '@/src/components/dashboard/DashboardTeams';
import DashboardDashboards from '@/src/components/dashboard/DashboardDashboards';
import DashboardWhiteboards from '@/src/components/dashboard/DashboardWhiteboards';
import DashboardForms from '@/src/components/dashboard/DashboardForms';
...
export default function IndividualDashboardPage() {
  ...
  // Guard: Prüfe, ob User Individual-Plan hat (oder Admin ist)
  useEffect(() => {
    if (userLoaded && user) {
      ...
      if (plan !== 'individual') {
        // User hat nicht den Individual-Plan → Redirect zu seinem Dashboard
        if (plan === 'starter') {
  ...
  // Funktion zum Neuladen des Users (für DashboardMore)
  const checkUser = async () => {
    // Bei Clerk wird der User automatisch aktualisiert
    // Diese Funktion ist hauptsächlich für Kompatibilität mit DashboardMore
    if (user) {
      // Optional: User-Daten neu laden, falls nötig
      // Clerk aktualisiert automatisch, daher ist dies meist nicht nötig
    }
  };
  
  // Supabase Client mit Clerk-Token erstellen
  const supabase = useMemo(() => {
    const supabaseUrl = 'https://cywpgkrahaioosmewpms.supabase.co';
    const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN5d3Bna3JhaGFpb29zbWV3cG1zIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjUyODg2NjQsImV4cCI6MjA4MDg2NDY2NH0.YWUWY86NrbfKt88oc4G1fsQvNdbocaQGKCPbr9cycHc';
  ...
  // user kommt jetzt von Clerk useUser Hook
  ...
  // Setze showCardsManager zurück, wenn der Benutzer die Karten-Seite verlässt
  useEffect(() => {
    if (activeMenu !== 'Karten') {
  ...
  const [userIsAdminState, setUserIsAdminState] = useState(false);
  ...
  const [timesheetEntries, setTimesheetEntries] = useState<Array<{
  ...
  }>>([
    {
      ...
      status: 'laufend',
      userId: user?.id,
      userName: user?.fullName || user?.emailAddresses[0]?.emailAddress || 'Unbekannt',
  ...
  useEffect(() => {
    ...
    // Prüfe ob User Admin ist (aus Clerk metadata)
    ...
    setUserIsAdminState(userIsAdmin);
    ...
    if (user.id && supabase) {
      loadWorkflows(user.id as string);
    }
    setLoading(false);
  ...
  const addWorkflow = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !supabase || !user?.id) return;
    const { data } = await supabase
      .from('workflows')
  ...
  const toggleGoalTask = (goalId: number, taskIndex: number) => {
    setGoals(
      goals.map((goal) => {
        if (goal.id === goalId) {
  ...
  const deleteWhiteboard = (whiteboardId: number) => {
  ...
  const deleteTimesheetEntry = (entryId: number) => {
    setTimesheetEntries(timesheetEntries.filter((e) => e.id !== entryId));
  };

  const toggleTimesheetStatus = (entryId: number) => {
    setTimesheetEntries(
  ...
  return (
    <DashboardLayout
      ...
      isCardsPage={activeMenu === 'Karten' || showCardsManager}
    >
      {activeMenu === 'Chat' ? (
        <DashboardChat
          input={input}
        ...
        <DashboardTeams
          ...
          user={user}
          allUsers={[
            { id: user?.id || 'user1', name: user?.fullName || user?.emailAddresses[0]?.emailAddress || 'Du', email: user?.emailAddresses[0]?.emailAddress || '' },
        ...
        <DashboardTimesheets
          ...
          user={user}
          isAdmin={userIsAdminState && showAdminTimesheets}
        ...
        <DashboardCards user={user} onCardClick={handleCardClick} isCardsPage={activeMenu === 'Karten' || showCardsManager} />
      ) : (
        <div
          style={{
            background: '#111827',
...
// Export ist bereits oben definiert
